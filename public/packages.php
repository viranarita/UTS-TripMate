...
// Fungsi untuk generate ID otomatis PKG001, PKG002, ...
function generateID() {
    global $conn;

    // Ambil ID terbesar saat ini
    $query = "SELECT MAX(package_id) as max_id FROM tb_Packages";
    ...
    return "PKG" . str_pad($newId, 3, '0', STR_PAD_LEFT);
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($id)) {
        ...
        $query = "INSERT INTO tb_Packages (package_id, name, details, price) 
                  VALUES ('$generatedID', '$name', '$details', '$price')";
    }
